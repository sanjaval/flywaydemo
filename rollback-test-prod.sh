#!/bin/bash
 ...
 docker run --rm -v $PWD/sql:/flyway/sql -v $PWD:/flyway/conf flyway/flyway undo -url="jdbc:postgresql://$pagilaHost:$pagilaPort/$databaseName" -user=$pagilaUser -password=$pagilaPassword
